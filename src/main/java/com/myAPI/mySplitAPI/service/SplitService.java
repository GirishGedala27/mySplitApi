...
public class SplitService {
	...
	public SplitResponse calculateSplit(SplitRequest splitRequest) {
		...
		if (isValidSplit(splitRequest)) {
			...
			splitRequest.getSplitGroup().getSplitGroupMembers().forEach(p -> {
			...
			throw new SplitException("500.BADSPLIT",
					"Error while performing split, group size and split value members count do not match");
		}
	}
	...
	private boolean isValidSplit(SplitRequest splitRequest) {
		...
		if (splitRequest.getSplitGroup().getSplitGroupMembers().size() == splitRequest.getSplitValue()
				.getTotalSplitGroupCount()) {
